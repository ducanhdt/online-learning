...
class ChatScreen extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    // const className = response.data.result.name
    // console.log({className});
    //const { email, room } = state || {};
    // const{email}=this.email;
    // const{room}=this.room;
    //console.log({$this.email,this.room});
    let token = "";
    ...
    try {
      token = await this.getToken(this.email);
    } catch {
    ...
    client.on("tokenAboutToExpire", async () => {
      const token = await this.getToken(this.email);
    ...
    try {
      const channel = await client.getChannelByUniqueName(this.room);
        ...
        <AppBar elevation={10}>
          <Toolbar>
            <Typography variant="h6">
              {`Room: ${this.room}, User: ${this.email}`}
            </Typography>
          </Toolbar>
        </AppBar>
        <CssBaseline />
